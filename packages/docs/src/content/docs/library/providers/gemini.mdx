...
<Aside type="caution">
  Gemini 3 Pro does **not** support `"medium"` or `"minimal"` thinking levels. llmist automatically maps unsupported levels to the nearest supported value.
</Aside>
...
<Aside type="tip">
  llmist auto-detects the specific Gemini model variant (2.5 vs 3, Pro vs Flash) and uses the correct native parameter and level mapping.
</Aside>
...
<Aside type="note">
  Caching requires a minimum of ~32K tokens of content to be eligible. Below that threshold, llmist gracefully falls back to uncached requests.
</Aside>

<Aside type="tip">
  llmist automatically detects content changes via hashing and creates a fresh cache when needed — you don't need to manage cache invalidation.
</Aside>
