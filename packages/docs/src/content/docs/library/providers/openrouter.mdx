...
<Aside type="tip">
  For long-running agent loops, caching can dramatically reduce costs. In a 94-iteration session processing 73 documents, enabling caching on Gemini Flash can reduce cumulative input tokens by 5-10x.
</Aside>
...
<Aside type="note">
  Reasoning behavior for models accessed through OpenRouter depends on the upstream provider's support. For first-class reasoning support with `withReasoning()`, use direct provider connections (OpenAI, Anthropic, Gemini). See the [Reasoning Models](/library/guides/reasoning-models/) guide.
</Aside>
