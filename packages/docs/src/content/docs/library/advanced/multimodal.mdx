...
<Tabs>
  <TabItem label="npm">
    ```bash
    # Image
    npx @llmist/cli image "A Windows 95 error dialog that says 'Success'" -m dall-e-3 -o success.png

    # Speech
    npx @llmist/cli speech "You've got mail!" -m tts-1 --voice nova -o aol.mp3
    ```
  </TabItem>
  <TabItem label="bun">
    ```bash
