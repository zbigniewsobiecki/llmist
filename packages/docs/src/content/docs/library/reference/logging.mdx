...
Each CLI session automatically creates its own log directory at `~/.llmist/logs/<session-name>/` with a memorable name like `sunny-falcon-42`. Session logs are stored in `session.log.jsonl` within that directory.
:::
...
<Tabs>
  <TabItem label="bun">
    ...
    bunx @llmist/cli agent "task" --log-level trace
    ```
  </TabItem>
  <TabItem label="npm">
    ```bash
