...
<Aside type="note">
  Each CLI session automatically creates a log directory at `~/.llmist/logs/<session-name>/` with a memorable name like `sunny-falcon-42`. Logs are stored in `session.log.jsonl` within that directory.
</Aside>
