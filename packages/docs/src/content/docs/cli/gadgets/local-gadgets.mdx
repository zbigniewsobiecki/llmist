...
Use the `/GadgetName` suffix to load a specific gadget from a file or package:

<Tabs>
  <TabItem label="bun">
    ```bash
    # Load only FloppyDisk from a file with multiple exports
    bunx @llmist/cli agent "Calculate floppies" --gadget ./retro-tools.ts/FloppyDisk

    # Load BrowseWeb subagent from Dhalsim package
...
<Aside type="note">
  The gadget name must be PascalCase (e.g., `MyGadget`, `BrowseWeb`). This matches the class name of the gadget.
</Aside>
