...
async function example3_ExternalStatusInjection() {
  ...
  try {
    await LLMist.createAgent()
      ...
      .withTrailingMessage(() => {
        return (
          `[CURRENT STATUS] Task: ${taskStatus} | System Load: ${systemLoad}. ` +
...
async function example5_CombiningWithHooks() {
  ...
  await LLMist.createAgent()
    ...
    .withHooks(
      HookPresets.merge(HookPresets.timing(), HookPresets.tokenTracking(), {
        observers: {
...
async function example6_EphemeralNature() {
    ...
    .withHooks({
      observers: {
        onLLMCallComplete: async (ctx) => {
          ...
          console.log(
            `   📝 Response includes [EPHEMERAL]: ${hasEphemeral ? "YES (bug!)" : "NO (correct!)"}`,
          );
  ...
  const anyContainsEphemeral = conversationHistory.some((msg) => msg.includes("[EPHEMERAL]"));
  console.log(
    `   Any history contains [EPHEMERAL]: ${anyContainsEphemeral ? "YES (bug!)" : "NO (correct!)"}`,
  );
}
